...
var PDF_DIR = import_path.default.join(process.cwd(), "PR_PDF");
var PO_PDF_DIR = import_path.default.join(process.cwd(), "PO_PDF");
if (!import_fs.default.existsSync(PDF_DIR)) {
  import_fs.default.mkdirSync(PDF_DIR);
}
if (!import_fs.default.existsSync(PO_PDF_DIR)) {
  import_fs.default.mkdirSync(PO_PDF_DIR);
}
...
var SPREADSHEET_ID = "1Ne5xeN2zEmScf9CVX5x9WguQZPM0Vk6dzQJ-n7xRfWU";
var getAuthClient = () => {
...
function createFallbackAuth(userAuth, getServiceAccountAuth) {
  ...
  fallbackAuth.request = async function(opts) {
    try {
      return await userAuth.request(opts);
    } catch (err) {
      const msg = err.message || "";
      const status = err.code || err.response?.status || 500;
      const isAuthError = status === 401 || status === 403 || msg.includes("Invalid Credentials") || msg.includes("invalid_token") || msg.includes("expired") || msg.includes("auth");
      if (isAuthError) {
        triggerFallback();
  ...
  fallbackAuth.getRequestHeaders = async function(url) {
    try {
...
var getSheets = (auth) => {
  const finalAuth = auth && !(auth instanceof import_googleapis.google.auth.JWT) ? createFallbackAuth(auth, getAuthClient) : auth;
  return import_googleapis.google.sheets({ version: "v4", auth: finalAuth });
};
var getDrive = (auth) => {
  const finalAuth = auth && !(auth instanceof import_googleapis.google.auth.JWT) ? createFallbackAuth(auth, getAuthClient) : auth;
  return import_googleapis.google.drive({ version: "v3", auth: finalAuth });
};
var getDocs = (auth) => {
  const finalAuth = auth && !(auth instanceof import_googleapis.google.auth.JWT) ? createFallbackAuth(auth, getAuthClient) : auth;
  return import_googleapis.google.docs({ version: "v1", auth: finalAuth });
};
var handleApiError = (res, error, context) => {
  const msg = error.message || "Unknown error";
  const status = error.code || error.response?.status || 500;
  console.error(`[${context}] Error (${status}): ${msg}`);
  const isAuthError = status === 401 || msg.includes("Invalid Credentials") || msg.includes("invalid_token") || msg.includes("expired") || msg.includes("auth");
  if (isAuthError) {
...
async function createPrPdf(prId, data, auth) {
  const fileName = `${prId.replace(/\//g, "_")}.pdf`;
  const filePath = import_path.default.join(PDF_DIR, fileName);
  if (!import_fs.default.existsSync(PDF_DIR)) {
    import_fs.default.mkdirSync(PDF_DIR, { recursive: true });
  }
  const totalQty = data.items.reduce((sum, item) => sum + Number(item.qty || 0), 0);
  const estimasiText = `( Stock untuk penjualan ` + data.items.map((item) => {
  ...
  try {
    ...
    const stream = import_fs.default.createWriteStream(filePath);
    ...
    const doc = new import_pdfkit.default({ margin: 30, size: "A4" });
    const stream = import_fs.default.createWriteStream(filePath);
    doc.pipe(stream);
    doc.rect(30, 30, 50, 40).fill("#1d4ed8");
    ...
    let curY = 205;
    doc.rect(30, curY, 535, 30).fill("#f1c232");
...
var GOOGLE_DRIVE_FOLDER_ID = "1GjYzgLWqoCt6FzihhD6q5xsv0MYYRb0c";
var GOOGLE_DRIVE_PO_FOLDER_ID = "1N6GwnHBE-RZcsTI1cbzmsWG-_zslN0qE";
var TEMPLATE_PR_ID = "1dNTmJEUxtXHyI044udxsKE6BBg1gnuJy55i2DzyuP9A";
var TEMPLATE_PO_ID = "14vYhIYIofui-HEY7rTo7oWD_DpGVPCLfj5atDKEpRio";
async function createPoPdf(poNo, data, auth) {
  const fileName = `${poNo.replace(/\//g, "_")}.pdf`;
  const filePath = import_path.default.join(PO_PDF_DIR, fileName);
  if (!import_fs.default.existsSync(PO_PDF_DIR)) {
    import_fs.default.mkdirSync(PO_PDF_DIR, { recursive: true });
  }
...
${info.address}`;
    ...
    const requests = [
      ...
      { replaceAllText: { containsText: { text: "{{TANGGAL}}", matchCase: false }, replaceText: (/* @__PURE__ */ new Date()).toLocaleDateString("id-ID") } },
...
async function uploadToDrive(filePath, fileName, auth, folderId) {
...
var isManagerOrDirector = (role, divisionCode) => {
  const roleUp = String(role || "").toUpperCase();
  const divUp = String(divisionCode || "").toUpperCase();
  return roleUp.includes("MANAGER") || roleUp.includes("MANAJER") || roleUp.includes("MGR") || roleUp.includes("KABAG") || roleUp.includes("DIREKTUR") || roleUp.includes("DIREKSI") || roleUp.includes("DIR") || roleUp.includes("KADIV") || divUp === "MGR" || divUp === "DIR";
};
var isAdmin = (role) => {
  const roleUp = String(role || "").toUpperCase();
...
function getWaToken() {
  const configPath = import_path.default.join(process.cwd(), "config_wa.json");
  ...
  if (!token && import_fs.default.existsSync(".env.example")) {
    try {
      const exampleContent = import_fs.default.readFileSync(".env.example", "utf-8");
...
async function sendWhatsApp(target, message, recipientName = "Unknown", recipientRole = "Unknown", messageType = "TEST_DIAGNOSTIC") {
...
async function notifyNewPR(prId, data, req) {
  try {
    ...
    const isManagerOnly = (role) => {
      const r = String(role || "").toUpperCase();
      return (r.includes("MANAGER") || r.includes("MANAJER") || r.includes("MGR") || r.includes("KABAG") || r.includes("KADIV")) && !(r.includes("DIREKTUR") || r.includes("DIREKSI") || r.includes("DIR"));
    };
    ...
    const firstItemAndQty = data.items && data.items.length > 0 ? `${data.items[0].itemName} (Qty: ${data.items[0].qty})` : "";
    const itemsCount = data.items ? data.items.length : 0;
    const itemSummary = itemsCount > 1 ? `${firstItemAndQty} + ${itemsCount - 1} item lainnya` : firstItemAndQty;
    ...
    for (const user of recipients) {
      const message = `\u{1F514} *Notifikasi PR Baru* \u{1F514}
...
\u{1F517} ${approvalLink}

Terima kasih.

\u{1F4A1} *Tips Pengguna HP (iOS / WhatsApp):*
...
async function notifyDirekturPR(prId, prDetails, approverName, req) {
  try {
    const users = await getNotificationUsers();
    const isDirekturOnly = (role) => {
      const r = String(role || "").toUpperCase();
    ...
    const firstItemAndQty = prDetails.items && prDetails.items.length > 0 ? `${prDetails.items[0].itemName} (Qty: ${prDetails.items[0].qty})` : "";
    ...
    const approvalLink = `${appBaseUrl}/?tab=approvals`;
    for (const user of recipients) {
      const message = `\u{1F514} Notifikasi Persetujuan Direktur \u{1F514}
...
async function notifyPRApprovalChange(prId, approverName, approverRole, status, isRejected = false, reason = "", requesterName, req) {
  try {
    const users = await getNotificationUsers();
    const targetRequester = String(requesterName || "").toUpperCase().trim();
    const recipients = users.filter((u) => u.wa && (String(u.displayName || "").toUpperCase().trim() === targetRequester || String(u.username || "").toUpperCase().trim() === targetRequester));
    if (recipients.length === 0) {
...
> Sent via fonnte.com`;
        ...
        message = `\u{1F4E2} *Notifikasi Persetujuan PR* \u{1F4E2}
...
async function syncPrDetailRow(prId, auth) {
  const sheets = getSheets(auth);
  try {
  ...
  try {
    ...
    if (userRow) {
      return res.json({
        ...
        user: {
          username: userRow[0] || "",
          displayName: userRow[2] || userRow[0] || "",
          // Fallback to username if FULL_NAME is empty
          division: userRow[3] || "CS",
          // Default division
          divisionCode: userRow[4] || "CS",
...
app.get("/api/stats", async (req, res) => {
  try {
    ...
    const poRes = await sheets.spreadsheets.values.get({
      spreadsheetId: SPREADSHEET_ID,
      range: "Rekap_PO!A2:O"
      // Updated range to include O (Division)
    });
    ...
    const getUniqueCountByStatus = (statuses) => {
      ...
      return Object.values(prGroups).filter((rows) => {
    ...
    const uniquePosWaitingReceive = new Set(
      prRows.filter((row) => String(row[11] || "").trim().toUpperCase() === "WAITING RECEIVE").map((row) => row[15]).filter(Boolean)
    ).size;
    ...
    const getTop = (rows, index, limit = 5) => {
      ...
      rows.forEach((row) => {
        ...
        if (val) {
          if (!counts[val]) {
            counts[val] = { prIds: /* @__PURE__ */ new Set(), totalQty: 0 };
          }
          if (prId) {
    ...
    stats.topSuppliers = getTop(prRows, 4);
    stats.topDivisions = getTop(prRows, 3);
    stats.topItems = getTop(prRows, 5, 10);
    res.json(stats);
  } catch (error) {
...
app.post(["/api/pr", "/api/pr/create"], async (req, res) => {
  try {
    ...
    const rowsToAppend = req.body.items.map((item) => [
      ...
      // Column J (index 9)
      req.body.notes || "",
      // Column K (index 10) - Overall Notes
...
app.get("/api/wa-diagnostics", async (req, res) => {
  try {
    ...
    const tokenPreview = token ? token.length > 15 ? `${token.slice(0, 8)}...${token.slice(-8)} (Length: ${token.length})` : `Short Token (${token.length})` : "Not Configured";
    const users = await getNotificationUsers();
...
app.post("/api/wa-save-token", async (req, res) => {
  try {
    ...
    const configPath = import_path.default.join(process.cwd(), "config_wa.json");
    import_fs.default.writeFileSync(configPath, JSON.stringify({ WA_API_TOKEN: token.trim() }, null, 2), "utf-8");
...
app.post("/api/pr/approve", async (req, res) => {
  try {
    ...
    const matchingIndices = rows.map((row, index) => row[0] && String(row[0]).trim() === String(prId).trim() ? index : -1).filter((index) => index !== -1);
...
app.post(["/api/po", "/api/po/create"], async (req, res) => {
  try {
    ...
    const poFilePath = import_path.default.join(PO_PDF_DIR, poFileName);
    uploadToDrive(poFilePath, poFileName, auth, GOOGLE_DRIVE_PO_FOLDER_ID).then(async (driveLink) => {
      ...
      if (driveLink) {
        try {
          ...
          const poRows2 = currentPoData.data.values || [];
          for (let i = 0; i < poRows2.length; i++) {
            if (poRows2[i][0] === poNo) {
              await sheets.spreadsheets.values.update({
                spreadsheetId: SPREADSHEET_ID,
    ...
    for (const item of items) {
      ...
      const rowIndex = prTable.findIndex(
        (row) => row[0] && String(row[0]).trim().toUpperCase() === cleanPrId && row[5] && String(row[5]).trim().toUpperCase() === cleanItemName
      );
      if (rowIndex !== -1) {
        await sheets.spreadsheets.values.update({
          spreadsheetId: SPREADSHEET_ID,
...
app.post("/api/pr/finish", async (req, res) => {
  try {
    ...
    const rows = response.data.values || [];
    const cleanPrId = String(prId || "").trim().toUpperCase();
    const matchingIndices = rows.map((row, index) => row[0] && String(row[0]).trim().toUpperCase() === cleanPrId ? index : -1).filter((index) => index !== -1);
    for (const rowIndex of matchingIndices) {
      await sheets.spreadsheets.values.update({
        spreadsheetId: SPREADSHEET_ID,
        range: `Rekap_PR!L${rowIndex + 1}`,
...
app.put("/api/pr/:index", async (req, res) => {
  try {
    const auth = getAuthFromRequest(req);
    const sheets = getSheets(auth);
    const editIndex = parseInt(req.params.index);
...
async function getSheetId(title, auth) {
  const sheets = getSheets(auth);
...
app.get("/api/pdf/pr/:id", async (req, res) => {
  ...
  if (import_fs.default.existsSync(filePath)) {
    ...
    return import_fs.default.createReadStream(filePath).pipe(res);
  }
  ...
  try {
    ...
    if (files && files.length > 0) {
      ...
      res.setHeader("Content-Security-Policy", "frame-ancestors 'self' *;");
      res.removeHeader("X-Frame-Options");
      const localStream = import_fs.default.createWriteStream(filePath);
